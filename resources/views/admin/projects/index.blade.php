            ...
            <div class="col-12">
                ...
                <div class="card card-outline card-primary">
                    <div class="card-header d-flex flex-wrap align-items-center gap-2">
                        <h3 class="card-title"><i class="fa-solid fa-folder-open me-2" aria-hidden="true"></i>{{ $title }}</h3>
                        @if(auth()->user()->isAdmin() || auth()->user()->isProjectAdmin())
                        <div class="card-tools ms-auto">
                            <a href="{{ route('admin.projects.create') }}"
                               class="btn btn-primary btn-sm">
                                <span class="fas fa-plus me-1"></span> {{ __('frontend.str.projects.create') }}
                            </a>
                        </div>
                        @endif
                    </div>
                    <div class="card-body">
                        <table id="itemList" class="table table-striped table-hover align-middle w-100">
                            <thead>
                            <tr>
                                <th>{{ __('frontend.str.projects.name') }}</th>
                                <th>{{ __('frontend.str.projects.description') }}</th>
                                <th>{{ __('frontend.str.projects.owner') }}</th>
                                <th>{{ __('frontend.str.projects.status') }}</th>
                                <th>{{ __('frontend.str.projects.members') }}</th>
        ...
        $(function () {
            ...
            $('#itemList').on('click', '.deleteRow', function () {
                ...
                Swal.fire({
                    ...
                    text: @json(__('frontend.str.projects.delete_confirmation')),
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ url('projects/destroy') }}/' + rowid,
                            type: "POST",
                            dataType: "html",
                            data: {_method: 'DELETE'},
                            ...
                            error: function (xhr, ajaxOptions, thrownError) {
                                Swal.fire({
                                    title: "{{ __('frontend.msg.error_deleting') }}",
                                    text: @json(__('frontend.msg.try_again')),
                                    icon: 'error'
                                });
                            }
                        });
